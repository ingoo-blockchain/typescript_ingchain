...
export class Chain {
    ...
    isValidNewBlock(_newBlock: Block, _previousBlock: Block): Boolean {
        switch (true) {
            ...
            case createHash(_previousBlock) !== _previousBlock.hash:
    ...
    isValidChain(_newChain: Block[]): Boolean {
        ...
        if (!Block.isValidGenesis(currentChains[0])) return false
        if (_newChain.length <= currentChains.length) return false
